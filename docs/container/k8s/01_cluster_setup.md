...
description: Debian / Fedora 上に Kubernetes クラスタを構築する
#sidebar_position: 0
#id: home
#slug: /my-custom-url
---
...
- Fedora / RHEL 等
    ...
    # Worker Nodeノード
    sudo firewall-cmd --permanent --add-port=10248/tcp       # kubelet healthz
    sudo firewall-cmd --permanent --add-port=10250/tcp       # kubelet API
...
:::tip バージョンについて
バージョン番号（`v1.36`）は、適宜、最新安定版を [Kubernetesリリースページ](https://kubernetes.io/releases/) で確認して変更してください。
:::
...
### Control Planeの初期化 <u>(Masterノードのみ)</u>
...
### CNI (Calico) のインストール <u>(Masterノードのみ)</u>
...
### Worker Nodeの参加 <u>(Worker ノードのみ)</u>
...
# k8s-node1   Ready    control-plane   10m   v1.36.x
